...
export const DiplomaticPass: React.FC<DiplomaticPassProps> = ({ formData, currentStep, isSuccess }) => {
  ...
  // Real-time Holographic Sheen Gradient tracking (aligned with cursor positions)
  const sheenX = useTransform(x, [-0.5, 0.5], ['200%', '-100%']);
  const sheenY = useTransform(y, [-0.5, 0.5], ['200%', '-100%']);
  ...
  return (
    <div className="w-full select-none justify-center flex py-4 md:py-8 lg:py-12">
      <div 
        ref={containerRef}
      ...
      >
        ...
        <div className="absolute -inset-2 bg-gradient-to-r from-primary/30 to-secondary/30 rounded-[2.5rem] blur-2xl opacity-40 group-hover:opacity-75 transition-opacity duration-1000" />
        
        {/* Holographic 3D Tilt Wrapper */}
        <motion.div 
          style={{ rotateX, rotateY, transformStyle: "preserve-3d" }}
          className="relative w-full aspect-[1.58/1] min-h-[250px] sm:min-h-[280px] bg-white/5 dark:bg-[#0c0d12]/60 border border-white/20 dark:border-white/10 rounded-[2.2rem] p-6 flex flex-col justify-between overflow-hidden shadow-[0_25px_60px_-15px_rgba(0,0,0,0.6)] backdrop-blur-2xl ring-1 ring-white/15"
        >
          ...
          <div className="relative z-10 flex items-start justify-between">
            <div className="flex items-center gap-3">
              <div className="relative w-10 h-10 rounded-xl bg-primary/15 border border-primary/20 flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform duration-500">
                <Shield className="w-5 h-5 text-primary animate-pulse" />
              </div>
              <div className="flex flex-col text-left">
                <span className="text-xs font-bold tracking-widest text-primary font-headline uppercase">DHMMUN 2026</span>
                <span className="text-[9px] text-on-surface-variant/70 font-mono tracking-wider font-bold">DIPLOMATIC CREDENTIAL</span>
              </div>
            </div>
            <div className="flex flex-col items-end text-right font-mono">
              <span className="text-[8px] bg-primary/10 text-primary border border-primary/25 rounded px-1.5 py-0.5 tracking-wider font-bold">
                {getClearanceLevel()}
              </span>
              <span className="text-[7px] text-on-surface-variant/40 mt-1 uppercase tracking-widest">Global Sec. ID</span>
            </div>
          </div>
          ...
          <div className="relative z-10 grid grid-cols-12 gap-4 items-center">
            ...
            <div className="col-span-3 flex justify-center">
              <div className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-surface-container/60 border border-outline-variant/30 flex items-center justify-center overflow-hidden hover:rotate-3 transition-transform duration-300">
                {formData.firstName || formData.lastName ? (
                  <div className="flex flex-col items-center justify-center">
                    <span className="text-xl font-black text-primary font-headline">
                      {formData.firstName?.[0] || ''}{formData.lastName?.[0] || ''}
                    </span>
                    <Fingerprint className="w-4 h-4 text-secondary/70 absolute bottom-1 right-1 opacity-70" />
                  </div>
                ) : (
                  <Fingerprint className="w-8 h-8 text-on-surface-variant/30 animate-[pulse_2s_infinite]" />
                )}
            ...
            <div className="col-span-6 flex flex-col text-left gap-1">
              <div className="text-sm sm:text-base font-black tracking-tight text-on-surface font-headline truncate max-w-[170px]">
                {formData.firstName || formData.lastName ? `${formData.firstName} ${formData.lastName}` : "UNASSIGNED APPLICANT"}
              </div>
              <div className="text-[10px] text-on-surface-variant/80 truncate font-mono max-w-[170px]">
              ...
              <div className="flex items-center gap-1.5 text-[9px] text-on-surface-variant/60">
                ...
                <span className="truncate max-w-[150px] font-medium font-body">
            ...
            <div className="col-span-3 flex justify-end">
              <div className="p-1 px-1.5 bg-white rounded-lg border border-white/20 filter invert opacity-80 hover:opacity-100 transition-opacity">
          ...
          <div className="relative z-10 pt-2 border-t border-dashed border-outline-variant/30 flex items-center justify-between text-[9px] font-mono">
            <div className="flex flex-col text-left max-w-[65%]">
              <div className="text-[7px] text-on-surface-variant/50 uppercase tracking-widest font-bold">Assigned Committee Allocation</div>
              <div className="text-[10px] text-primary font-bold font-headline truncate mt-0.5 flex items-center gap-1">
                <Award className="w-3 h-3 shrink-0 text-primary" />
                {formData.committee ? getCommitteeLabel(formData.committee) : "Mission Pending"}
              </div>
            </div>
            
            <div className="flex flex-col text-right max-w-[35%]">
              <div className="text-[7px] text-on-surface-variant/50 uppercase tracking-widest font-bold">Designated Sovereign</div>
              <div className="text-[10px] text-secondary font-bold font-headline truncate mt-0.5 flex items-center gap-1 justify-end">
          ...
          <div className="absolute bottom-1 left-6 text-[7px] text-on-surface-variant/30 font-mono tracking-widest">
            {getSerialNumber()}
          </div>
        </motion.div>
      </div>
